...
// API to create a Stripe Checkout Session for an appointment
const paymentSTRIPE = async (req, res) => {
    try {
        ...
        const amountInSmallestUnit = Math.round(appointmentData.amount * 100) // e.g., INR paisa
...
// API to verify Stripe Checkout Session and mark appointment paid
const verifySTRIPE = async (req, res) => {
    try {
        ...
        if (session && (session.payment_status === 'paid' || (session.payment_intent && session.payment_intent.status === 'succeeded'))) {
...
export { bookAppointment, cancelAppointment, getProfile, listAppointment, loginUser, paymentSTRIPE, registerUser, updateProfile, verifySTRIPE };
